...
def GetFileName(file_path, max_length=130, add_hash=True):
    """
    不同系统适配
    ...
    if len(file_name) > max_length:
        # 截取最大长度
        trimmed_name = file_name[:max_length]

        # 添加 hash 防止重名或文件名过短重复
        if add_hash:
            hash_suffix = hashlib.md5(file_name.encode('utf-8')).hexdigest()[:8]
            file_name = f"{trimmed_name}_{hash_suffix}"
        else:
